...
class BDWKDOC(BaiduWK):
    ...
    def get_pure_addr_list(self):
        ...
        # 从源码中批量提取数据url
        all_addr = re.findall(r'wkbos\.bdimg\.com.*?json.*?expire.*?\}',
                              source_html)
        pure_addr_list = list()
        # 获取文档标题
...
class BDWKPPT(BaiduWK):
    ...
    def get_ppt_json_info(self):
        ...
        pure_str_source_json = re.match(r'.*?\((.*?)\)',
                                        str_source_json).group(1)
        ...
        for img_url in self.all_img_url:
            # print(img_url)
            print("正在获取第%d页资源(剩余%d页)" % (img_url[1],
                                         len(self.all_img_url) - img_url[1]))
